    ...
    site.getEmployeeOvertime = function (req, paySlip, callback) {
        ...
        app.$collection.findMany({ where: { 'employee.id': paySlip.employeeId, date: { $gte: d1, $lte: d2 }, active: true, requestStatus: 'accepted' } }, (err, docs) => {
            if (docs && docs.length) {
                docs.forEach((doc) => {
                    doc = { ...doc, ...paySlip };
                    let amount = paySlip.minuteSalary;
                    const overtime = {
                        appName: app.name,
                        date: doc.date,
                    ...
                    paySlip.overtimeValue += overtime.value;
    ...
    if (app.allowRoute) {
        if (app.allowRouteGet) {
            site.get(
                ...
                (req, res) => {
                    res.render(app.name + '/index.html', { title: app.name, appName: req.word("Overtime Requests"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
                }
            );
        }
