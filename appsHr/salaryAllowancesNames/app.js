  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Salary Allowances Names"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
        ...
        if (site.isFileExistsSync(response.file.filepath)) {
          ...
          if (Array.isArray(docs)) {
            ...
            docs.forEach((doc) => {
              ...
              let addToBasicSalary = doc.addToBasicSalary || doc['add to basic salary'] || false;
