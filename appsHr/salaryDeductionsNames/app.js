  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Salary Deductions Names"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
              ...
              app.$collection.find({ nameEn }, (err, doc) => {
                if (!doc) {
                  ...
                  let newDoc = {
                    ...
                    nameAr,
                    nameEn,
                    image: { url: '/images/salaryDeductionsNames.png' },
                    active: true,
                  };
