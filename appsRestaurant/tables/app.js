  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Tables"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
    ...
    site.post({ name: `/api/${app.name}/select`, public: true }, (req, res) => {
      let response = {
        done: false,
      };

      let _data = req.data;
      if (_data.oldTable && _data.newTable.id == _data.oldTable.id) {
        response.error = 'The table is reserved';
        res.json(response);
        return;
      }

      let newTable = app.memoryList.find((itm) => itm.id == _data.newTable.id);
      if (newTable) {
        if (newTable.busy) {
          response.error = 'The table is reserved';
          res.json(response);
          ...
          newTable.busy = true;
          app.update(newTable, (err, result) => {
            if (err) {
              response.error = err.message;
              res.json(response);
              return;
            } else {
              let oldTable = app.memoryList.find((itm) => _data.oldTable && itm.id == _data.oldTable.id);
              if (oldTable) {
                oldTable.busy = false;
                app.update(oldTable, (err1, result) => {
                  if (err1) {
