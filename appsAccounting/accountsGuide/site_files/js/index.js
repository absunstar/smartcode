app.controller('accountsGuide', function ($scope, $http, $timeout) {
    ...
    $scope.showAdd = function (_item) {
        ...
        if ($scope.setting.accountsSetting && $scope.setting.accountsSetting.autoGenerateAccountsGuideAndCostCenterCode) {
            $scope.item.lengthLevel = $scope.setting.accountsSetting.lengthLevel || 0;
        }
    ...
    $scope.getCategoryList = function () {
        $scope.error = '';
        $timeout(() => {
            if ($scope.setting.accountsSetting.linkGlWithIncomeStatementAndBudget) {
