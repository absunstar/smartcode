  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Safes Adjusting"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
      ...
      site.post({ name: `/api/${app.name}/approve`, require: { permissions: ['login'] } }, (req, res) => {
        ...
        app.update(_data, (err, result) => {
          if (!err) {
            ...
            let obj = {
              date: new Date(),
              voucherType: site.vouchersTypes[10],
