app.controller('printersPaths', function ($scope, $http, $timeout) {
  $scope.baseURL = '';
  $scope.appName = 'printersPaths';
  $scope.modalID = '#printersPathsManageModal';
  $scope.modalSearchID = '#printersPathsSearchModal';
  $scope.mode = 'add';
  $scope._search = {};
  $scope.structure = {
    ipDevice: '127.0.0.1',
  ...
  $scope.getPrinterTypeList = function () {
    ...
    $http({
      method: 'POST',
      url: '/api/printersTypes',
    }).then(
      function (response) {
        $scope.busy = false;
        if (response.data.done && response.data.list.length > 0) {
          $scope.printerTypeList = response.data.list;
        }
      },
      function (err) {
        $scope.busy = false;
