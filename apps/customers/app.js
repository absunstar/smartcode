  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Customers"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
        ...
        if (site.isFileExistsSync(response.file.filepath)) {
          ...
          if (Array.isArray(docs)) {
            ...
            docs.forEach((doc) => {
              ...
              let commercialCustomer = false;
              if (doc.commercialCustomer === 1) {
              ...
              let newDoc = {
                ...
                email: doc.email,
                mobile: '0' + doc.mobile,
                type: site.usersTypesList[5],
                commercialCustomer,
                taxIdentificationNumber: doc.taxIdentificationNumber,
