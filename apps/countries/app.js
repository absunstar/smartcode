  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + "/index.html", { title: app.name, appName: req.word("Countries"), setting: site.getCompanySetting(req) }, { parser: "html", compres: true });
        }
      );
    }
    ...
    if (app.allowRouteAll) {
      site.post({ name: `/api/${app.name}/all`, public: true }, (req, res) => {
        ...
        let select = req.body.select || { id: 1, code: 1, nameEn: 1, nameAr: 1, image: 1, callingCode: 1 };
