  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Convert Units"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
    ...
    if (app.allowRouteApprove) {
      site.post({ name: `/api/${app.name}/approve`, require: { permissions: ['login'] } }, (req, res) => {
        ...
        if (errBatchList.length > 0) {
          ...
          response.error = `The (From) Batches Count is not correct in ( ${error} )`;
          res.json(response);
          return;
        }

        if (errToBatchList.length > 0) {
          let error = errToBatchList.map((m) => m).join('-');
          response.error = `The (To) Batches Count is not correct in ( ${error} )`;
          res.json(response);
          return;
        }
