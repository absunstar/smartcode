module.exports = function init(site) {
  ...
  if (app.allowRoute) {
    ...
    if (app.allowRouteAdd) {
      site.post(
        ...
        (req, res) => {
          ...
          site.getBatchesToSalesAuto(
            { store: _data.store, items: _data.itemsList },
            (callbackItems) => {
              _data.itemsList.forEach((_item) => {
                if (_item.hasMedicalData && _data.salesType.code != "company") {
                  if (
                    !_item.medicineDuration ||
                    !_item.medicineFrequency ||
                    !_item.medicineRoute
                  ) {
                    let itemName =
                      req.session.lang == "Ar" ? _item.nameAr : _item.nameEn;
                ...
                if (
                  ...
                  if (_item.batchesList) {
                    if (storesSetting.workFifo) {
                      _item.$batchCount = _item.batchesList.reduce(
                        (a, b) => a + b.count,
                        0
                      );

                      if (_item.$batchCount != _item.count) {
                        let itemIndex = callbackItems.findIndex(
                          (itm) => itm.id === _item.id
                        );
                        if (itemIndex !== -1) {
                          _item.batchesList =
                            callbackItems[itemIndex].batchesList;
                        }
                      }
                      _item.$batchCount = _item.batchesList.reduce(
                        (a, b) => a + b.count,
              ...
              site.checkOverDraft(req, overDraftObj, (overDraftCb) => {
                if (!overDraftCb.done) {
                  let error = "";
                  error = overDraftCb.refuseList
                    .map((m) =>
                      req.session.lang == "Ar" ? m.nameAr : m.nameEn
                    )
                    .join("-");
                ...
                if (
                  _data.invoiceType.id == 1 &&
                  accountsSetting.linkAccountsToStores
                ) {
                  if (
                    site.toNumber(_data.paymentList[0].paidByCustomer) <
                      site.toNumber(_data.totalNet) &&
                    _data.paymentList.length == 1
                  ) {
                    response.error =
                      "Must Paid By Customer greater than or equal to total Net";
                    res.json(response);
                    return;
                  }
                  ...
                  if (_data.invoiceType.id == 2) {
                    if (
                      _data.installmentsList &&
                      _data.installmentsList.length > 0
                    ) {
                      let totalInstallment = _data.installmentsList.reduce(
                        (a, b) => a + b.amount,
                        0
                      );
            ...
            (err, docs) => {
              if (req.body.claims && docs) {
                ...
                docs.forEach((_doc) => {
                  if (
                    _doc.itemsList &&
                    _doc.itemsList.length > 0 &&
                    _doc.doctorDeskTop &&
                    _doc.doctorDeskTop.id
                  ) {
                    _doc.itemsList.forEach((_s) => {
                      let obj = {
                        ...
                        Nationality:
                          _doc.doctorDeskTop.patient.nationality.nameEn,
                        ...
                        ICD10_Code1: "",
                        Claim_Type:
                          _doc.doctorDeskTop.type == "out" ? "O" : "I",
                        ...
                        Blood_pressure:
                          _doc.doctorDeskTop.bloodPressureHigh +
                          "/" +
                          _doc.doctorDeskTop.bloodPressureLow,
                        Height: _doc.doctorDeskTop.height,
                        Weight: _doc.doctorDeskTop.weight,
                        Pulse: _doc.doctorDeskTop.pulse,
                        Policy_Name:
                          _doc.doctorDeskTop.patient.insuranceCompany &&
                          _doc.doctorDeskTop.patient.insuranceCompany.id
                            ? _doc.doctorDeskTop.patient.insuranceCompany.nameEn
                            : "",
                        MediCode: "",
                        Notes: _s.notesAfter,
    ...
    if (app.allowRouteReport) {
      site.post(
        { name: `/api/${app.name}/report`, public: true },
        (req, res) => {
          let where = req.body.where || {};
          let select = req.body.select || {};
          let search = req.body.search || "";
  ...
  site.post(
    { name: `/api/${app.name}/approve`, require: { permissions: ["login"] } },
    (req, res) => {
      let response = {
        done: false,
      };
        ...
        (callbackItems) => {
          ...
          site.checkOverDraft(req, overDraftObj, (overDraftCb) => {
            if (!overDraftCb.done) {
              let error = "";
              error = overDraftCb.refuseList
                .map((m) => (req.session.lang == "Ar" ? m.nameAr : m.nameEn))
                .join("-");
              response.error = `Item Balance Insufficient ( ${error} )`;
              res.json(response);
              return;
            ...
            app.update(_data, (err, result) => {
              if (!err) {
                ...
                if (result.doc) {
                  ...
                  if (
                    result.doc.store.linkWithRasd &&
                    result.doc.store.rasdUser &&
                    result.doc.store.rasdPass
                  ) {
                    site.sendRasdData({
    ...
    app.all({ where, select }, (err, docs) => {
      ...
      if (!err && docs) {
        ...
        for (let i = 0; i < docs.length; i++) {
          ...
          if (
            weekDate.firstday.getDate() <= docs[i].date.getDate() &&
            weekDate.lastday.getDate() >= docs[i].date.getDate()
          ) {
            obj.week += 1;
          }
        }
  ...
  site.changeRemainPaidSalesInvoices = function (obj) {
    app.view({ id: obj.id }, (err, doc) => {
      if (!err && doc) {
        if (
          doc.invoiceType.id == 2 &&
          doc.installmentsList &&
          doc.installmentsList.length > 0
        ) {
          let index = doc.installmentsList.findIndex(
            (itm) => itm.date.toString() === obj.installment.date.toString()
          );
          if (index !== -1) {
