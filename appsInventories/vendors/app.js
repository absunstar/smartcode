    ...
    if (app.allowRoute) {
        if (app.allowRouteGet) {
            site.get(
                ...
                (req, res) => {
                    res.render(app.name + '/index.html', { title: app.name, appName: req.word("Vendors"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
                }
            );
        }
        ...
        if (app.allowRouteAdd) {
            site.post({ name: `/api/${app.name}/add`, require: { permissions: ['login'] } }, (req, res) => {
                ...
                _data.type = site.usersTypesList[6];
