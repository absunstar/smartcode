module.exports = function init(site) {
    ...
    app.active = function (_options, callback) {
        if (callback) {
            if (app.allowMemory) {
                callback(null, app.memoryList);
            } else {
    ...
    if (app.allowRoute) {
        if (app.allowRouteGet) {
            site.get(
                ...
                (req, res) => {
                    res.render(app.name + '/index.html', { title: app.name, appName: req.word("Active Substances"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
                }
            );
        }
