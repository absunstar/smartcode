app.controller('storesItems', function ($scope, $http, $timeout) {
  ...
  $scope.getItemsMedicalTypes = function () {
    $scope.busy = true;
    $scope.itemsMedicalTypesList = [];
    $http({
      method: 'POST',
      url: '/api/itemsMedicalTypes',
      data: {},
    }).then(
      function (response) {
        $scope.busy = false;
        if (response.data.done && response.data.list.length > 0) {
          $scope.itemsMedicalTypesList = response.data.list;
        }
      },
      function (err) {
        $scope.busy = false;
...
/*     if (!itemUnit.barcode) {
  ...
  $scope.validateData = function (_item) {
    ...
    if (_item.workByBatch && _item.validityDays < 1 && !_item.hasColorsData && !_item.hasSizesData) {
