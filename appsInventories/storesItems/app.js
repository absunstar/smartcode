module.exports = function init(site) {
            ...
            if (err) {
                ...
                if (unitIndex != -1) {
                    ...
                    if (storeIndex != -1) {
                        ...
                        let batchIndex = doc.unitsList[unitIndex].storesList[storeIndex].batchesList.findIndex((b) => b.code == where.code || b.sn == where.sn);
                        if (batchIndex != -1) {
                            ...
                            res.json({
                                ...
                                docs: doc.unitsList[unitIndex].storesList[storeIndex].batchesList.slice(0, 50),
                            });
                        }
                    } else {
                        res.json({
    ...
    site.post({ name: `/api/${app.name}/getBatchesSearch`, require: { permissions: ['login'] } }, (req, res) => {
        let where = req.body.where || {};
        app.view({ id: where.id }, (err, doc) => {
            if (err) {
                res.json({
                ...
                if (unitIndex != -1) {
                    ...
                    if (storeIndex != -1) {
                        ...
                        if (doc.workByBatch || doc.workByQrCode) {
                            doc[indexDoc].unitsList[unitIndex].storesList[storeIndex].batchesList = doc[indexDoc].unitsList[unitIndex].storesList[storeIndex].batchesList
                                .sort((a, b) => new Date(b.expiryDate) - new Date(a.expiryDate))
                                .reverse();
                        } else if (doc.workBySerial) {
                            doc[indexDoc].unitsList[unitIndex].storesList[storeIndex].batchesList = doc[indexDoc].unitsList[unitIndex].storesList[storeIndex].batchesList
                        ...
                        res.json({
                            ...
                            docs: doc[indexDoc].unitsList[unitIndex].storesList[storeIndex].batchesList.slice(0, 50),
                        });
