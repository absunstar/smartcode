module.exports = function init(site) {
    ...
    app.all({ where: { id: { $in: itemIds } } }, (err, docs) => {
      if (!err && docs) {
        ...
        for (let i = 0; i < obj.items.length; i++) {
          ...
          if (indexDoc != -1) {
            let unitIndex = docs[indexDoc].unitsList.findIndex(
              (unt) => unt.unit.id === item.unit.id
            );
            if (unitIndex != -1) {
              let storeIndex = docs[indexDoc].unitsList[
                unitIndex
              ].storesList.findIndex((st) => st.store.id === obj.store.id);
              if (storeIndex != -1) {
                docs[indexDoc].unitsList[unitIndex].storesList[
                  storeIndex
                ].batchesList =
                  docs[indexDoc].unitsList[unitIndex].storesList[storeIndex]
                    .batchesList || [];
                let batchesList = [];
                if (item.workByBatch || item.workByQrCode) {
                  docs[indexDoc].unitsList[unitIndex].storesList[
                    storeIndex
                  ].batchesList = docs[indexDoc].unitsList[
                    unitIndex
                  ].storesList[storeIndex].batchesList
                    .sort(
  ...
  site.editItemsBalance = function (_elm, screenName) {
    ...
    app.view({ id: _elm.id }, (err, doc) => {
      if (doc && doc.itemType.id == 1) {
        ...
        if (index != -1) {
          let storeIndex = doc.unitsList[index].storesList.findIndex(
            (s) => s.store && s.store.id === _elm.store.id
          );
          ...
          if (
            screenName === "purchaseOrders" ||
            screenName === "storesOpeningBalances"
          ) {
            if (screenName === "purchaseOrders") {
              doc.unitsList[index].storesList[storeIndex].purchaseCount +=
                _elm.count;
              doc.unitsList[index].storesList[storeIndex].bonusCount +=
                _elm.bonusCount;
              doc.unitsList[index].storesList[storeIndex].bonusPrice +=
                _elm.bonusPrice;
            } else {
              doc.unitsList[index].storesList[storeIndex].openingBalanceCount +=
                _elm.count;
              doc.unitsList[index].storesList[storeIndex].openingBalancePrice +=
                _elm.total;
            }
            ...
            const oldCost =
              selectedUnit.currentCount * selectedUnit.purchasePrice;
            const newCost = _elm.count * _elm.price;
            const totalCount = selectedUnit.currentCount + _elm.count;
            doc.unitsList[index].purchasePrice =
              (oldCost + newCost) / totalCount;
            doc.unitsList[index].purchasePrice = site.toNumber(
              doc.unitsList[index].purchasePrice
            );

            if (_elm.workByBatch || _elm.workBySerial || _elm.workByQrCode) {
              doc.unitsList[index].storesList[storeIndex] =
                site.handelAddBatches(
                  doc.unitsList[index].storesList[storeIndex],
                  _elm.batchesList
                );
            }
          } else if (screenName === "returnPurchaseOrders") {
            doc.unitsList[index].storesList[storeIndex].purchaseReturnCost +=
              _elm.cost;
            doc.unitsList[index].storesList[storeIndex].purchaseReturnCount +=
              _elm.count;
            doc.unitsList[index].storesList[storeIndex].purchaseReturnPrice +=
              _elm.total;
            doc.unitsList[index].storesList[storeIndex].bonusReturnCount +=
              _elm.bonusCount;
            doc.unitsList[index].storesList[storeIndex].bonusReturnPrice +=
              _elm.bonusPrice;
            if (_elm.workByBatch || _elm.workBySerial || _elm.workByQrCode) {
              doc.unitsList[index].storesList[storeIndex] =
                site.handelBalanceBatches(
                  doc.unitsList[index].storesList[storeIndex],
                  _elm.batchesList,
                  "-"
                );
            ...
            doc.unitsList[index].storesList[storeIndex].salesCount +=
              _elm.count;
            doc.unitsList[index].storesList[storeIndex].salesPrice +=
              _elm.total;
            doc.unitsList[index].storesList[storeIndex].lastSellingDate =
              new Date();
            if (_elm.workByBatch || _elm.workBySerial || _elm.workByQrCode) {
              doc.unitsList[index].storesList[storeIndex] =
                site.handelBalanceBatches(
            ...
            doc.unitsList[index].storesList[storeIndex].salesReturnCount +=
              _elm.count;
            doc.unitsList[index].storesList[storeIndex].salesReturnPrice +=
              _elm.total;
            if (_elm.workByBatch || _elm.workBySerial || _elm.workByQrCode) {
              doc.unitsList[index].storesList[storeIndex] =
                site.handelBalanceBatches(
            ...
            doc.unitsList[index].storesList[storeIndex].convertUnitFromCount +=
              _elm.count;
            doc.unitsList[index].storesList[storeIndex].convertUnitFromPrice +=
              _elm.total;
            let unitToIndex = doc.unitsList.findIndex(
              (s) => s.unit && s.unit.id === _elm.toUnit.id
            );
            if (unitToIndex != -1) {
              let storeIndexTo = doc.unitsList[
                unitToIndex
              ].storesList.findIndex(
                (s) => s.store && s.store.id === _elm.store.id
              );
              if (storeIndexTo == -1) {
                const newUitStore = site.setStoresItemsUnitStoreProperties();
              ...
              doc.unitsList[unitToIndex].storesList[
                storeIndexTo
              ].convertUnitToCount += _elm.toCount;
              doc.unitsList[unitToIndex].storesList[
                storeIndexTo
              ].convertUnitToPrice += _elm.toTotal;
              doc.unitsList[index].storesList[storeIndex] =
                site.handelBalanceBatches(
                  doc.unitsList[index].storesList[storeIndex],
                  _elm.batchesList,
              ...
              doc.unitsList[unitToIndex].storesList[storeIndexTo] =
                site.handelAddBatches(
                  doc.unitsList[unitToIndex].storesList[storeIndexTo],
                  _elm.toBatchesList
                );
            ...
            doc.unitsList[index].storesList[storeIndex].transferFromCount +=
              _elm.count;
            doc.unitsList[index].storesList[storeIndex].transferFromPrice +=
              _elm.total;

            let storeToIndex = doc.unitsList[index].storesList.findIndex(
              (s) => s.store && s.store.id === _elm.toStore.id
            );
            if (storeToIndex == -1) {
              const newUitStore = site.setStoresItemsUnitStoreProperties();
            ...
            doc.unitsList[index].storesList[storeToIndex].transferToCount +=
              _elm.count;
            doc.unitsList[index].storesList[storeToIndex].transferToPrice +=
              _elm.total;
            if (_elm.workByBatch || _elm.workBySerial || _elm.workByQrCode) {
              doc.unitsList[index].storesList[storeIndex] =
                site.handelBalanceBatches(
              ...
              doc.unitsList[index].storesList[storeToIndex] =
                site.handelAddBatches(
                  doc.unitsList[index].storesList[storeToIndex],
                  _elm.batchesList
                );
            ...
            doc.unitsList[index].storesList[storeIndex].damagedCount +=
              _elm.count;
            doc.unitsList[index].storesList[storeIndex].damagedPrice +=
              _elm.total;
            if (_elm.workByBatch || _elm.workBySerial || _elm.workByQrCode) {
              doc.unitsList[index].storesList[storeIndex] =
                site.handelBalanceBatches(
            ...
            if (_elm.countType == "in") {
              doc.unitsList[index].storesList[storeIndex].stockTakingInCount +=
                _elm.count;
              doc.unitsList[index].storesList[storeIndex].stockTakingInPrice +=
                _elm.total;
            } else if (_elm.countType == "out") {
              doc.unitsList[index].storesList[storeIndex].stockTakingOutCount +=
                _elm.count;
              doc.unitsList[index].storesList[storeIndex].stockTakingOutPrice +=
                _elm.total;
            }
  ...
  site.checkBarcode = function (barcodes, callback) {
    app.all(
      { where: { "unitsList.barcode": { $in: barcodes } } },
      (err, docs) => {
        if (docs) {
          let list = [];
          docs.forEach((_doc) => {
    ...
    item.unitsList.forEach((unt) => {
      ...
      unt.storesList.forEach((str) => {
        let totalIncome =
          str.purchaseCount +
          str.bonusCount +
          str.openingBalanceCount +
          str.unassembledCount +
          str.salesReturnCount +
          str.stockTakingInCount +
          str.transferToCount +
          str.convertUnitToCount;
        let totalOut =
          str.salesCount +
          str.purchaseReturnCount +
          str.damagedCount +
          str.assembledCount +
          str.stockTakingOutCount +
          str.transferFromCount +
          str.convertUnitFromCount +
      ...
      for (let i = 0; i < batchesList.length; i++) {
        ...
        if (b.count > 0) {
          if (obj.batchesList.length > 0) {
            let batchIndex = obj.batchesList.findIndex((_b) =>
              _b.sn ? _b.sn === b.sn : _b.code === b.code
            );
            if (batchIndex != -1) {
  ...
  site.handelBalanceBatches = function (obj, batchesList, type) {
    ...
    if (batchesList && batchesList.length > 0) {
      for (let i = 0; i < batchesList.length; i++) {
        let b = batchesList[i];
        let batchIndex = obj.batchesList.findIndex(
          (_b) => (_b.sn && _b.sn === b.sn) || _b.code === b.code
        );
        if (batchIndex != -1) {
          if (type == "+") {
            obj.batchesList[batchIndex].count =
              obj.batchesList[batchIndex].count + b.count;
          } else if (type == "-") {
            obj.batchesList[batchIndex].count =
              obj.batchesList[batchIndex].count - b.count;
              ...
              if (docs[0].nameAr == _data.nameAr) {
                ...
                docs.forEach((_doc) => {
                  _data.unitsList.forEach((_unit) => {
                    if (
                      _doc.unitsList.some((u) => u.barcode == _unit.barcode)
                    ) {
        ...
        $itemsFile.findMany(
          ...
          (err, oldDocs) => {
            ...
            oldDocs.forEach((_oldDoc) => {
              let unitExists = unitsList.some(
                (u) => u.name_ar === _oldDoc.unit
              );
              if (!unitExists) {
                unitsList.push({
              ...
              let groupExists = itemsGroupList.some(
                (g) => g.name_ar === _oldDoc.category_name_ar
              );
              if (!groupExists) {
                itemsGroupList.push({
      ...
      site.post(`api/${app.name}/importIdfList`, (req, res) => {
        ...
        if (site.isFileExistsSync(response.file.filepath)) {
          ...
          if (Array.isArray(docs)) {
            ...
            docs.forEach((doc) => {
              app.$collection.find(
                { where: { nameEn: String(doc["Trade Name"]).trim() } },
                (err, exisitDoc) => {
                  if (exisitDoc) {
                    ...
                    let newDoc = {
                      ...
                      sfdaCodeList: [
                        { sfdaCode: String(doc["Register Number"]).trim() },
                      ],
                      gtinList,
                      unitsList: [],
                      medicalInformations: {},
              ...
              app.$collection.find(
                ...
                (err, exisitDoc) => {
                  if (exisitDoc) {
                    ...
                    const valid = app.validateExisitDrugInBusyList(
                      exisitDoc.id
                    );
                    if (valid) {
                      setTimeout(() => {
                    ...
                    const sfdaIndex = exisitDoc.sfdaCodeList.findIndex(
                      (sfda) =>
                        sfda.sfdaCode == String(doc["RegisterNumber"]).trim()
                    );
                    ...
                    if (sfdaIndex !== -1) {
                      ...
                      if (elementsList.length == 1) {
                        ...
                        activeSubstanceItem.concentration =
                          strengthList[0] +
                          " " +
                          String(doc["StrengthUnit"]).trim();
                      } else {
                        elementsList.forEach((elem, index) => {
                          ...
                          if (strengthList.length == 1) {
                            ...
                            let item = strengthList.findIndex(() =>
                              strengthList.indexOf(index)
                            );
                            activeSubstanceItem.concentration =
                              strengthList[item] +
                              " " +
                          ...
                          exisitDoc.medicalInformations.activeSubstancesList =
                        ...
                        (err, foundUnit) => {
                          const unit = {
                            ...
                            nameAr: foundUnit.nameAr
                              ? foundUnit?.nameAr.trim()
                              : "",
                            nameEn: foundUnit.nameEn
                              ? foundUnit?.nameEn.trim()
                              : "",
                            image: foundUnit?.image,
                          };
                          const conversion = Number(
                            String(doc["PackageSize"]).trim()
                          );
                          const salesPrice = Number(
                            String(doc["Public price"]).trim()
                          );
                          exisitDoc.unitsList = [
                            {
                              unit,
                              ...
                              purchasePriceList: [],
                              salesPriceList: [
                                { price: salesPrice, date: new Date() },
                              ],
                              discountType: "value",
                              storesList: [],
                              active: true,
                          ...
                          app.update(exisitDoc, () => {
                            ...
                            const itemIndex =
                              app.importHumanDrugsbusyList.findIndex(
                                () => exisitDoc.id
                              );
                            if (itemIndex != -1) {
  ...
  site.post(
    { name: `/api/${app.name}/getBatch`, require: { permissions: ["login"] } },
    (req, res) => {
      let where = req.body.where || {};
      app.view({ id: where.id }, (err, doc) => {
        if (err) {
          ...
          let unitIndex = doc.unitsList.findIndex(
            (unt) => unt.unit.id === where.unitId
          );
          if (unitIndex != -1) {
            let storeIndex = doc.unitsList[unitIndex].storesList.findIndex(
              (st) => st.store.id === where.storeId
            );
            if (storeIndex != -1) {
              doc.unitsList[unitIndex].storesList[storeIndex].batchesList =
                doc.unitsList[unitIndex].storesList[storeIndex].batchesList ||
                [];
              let batchIndex = doc.unitsList[unitIndex].storesList[
                storeIndex
              ].batchesList.findIndex((b) =>
                where.sn ? b.sn == where.sn : b.code == where.code
              );
              if (batchIndex != -1) {
                doc.unitsList[unitIndex].storesList[storeIndex].batchesList[
                  batchIndex
                ...
                let batch = {
                  ...doc.unitsList[unitIndex].storesList[storeIndex]
                    .batchesList[batchIndex],
                };
  ...
  site.post(
    ...
    (req, res) => {
      ...
      app.all({ where: { id: { $in: itemIds } } }, (err, docs) => {
        site.getSalesInvoice(
          { "doctorDeskTop.id": req.body.doctorDeskTopId },
          (salesInvoicesData) => {
            let obj = { done: true };
            let appInsuranceContract = site.getApp("insuranceContracts");
            let insuranceContract = appInsuranceContract.memoryList.find(
              (_c) => _c.id == req.body.insuranceCompanyId
            );

            if (
              insuranceContract &&
              new Date(insuranceContract.startDate) <= new Date() &&
              new Date(insuranceContract.endDate) >= new Date()
            ) {
              let insuranceClass = insuranceContract.insuranceClassesList.find(
                (_c) => _c.id == _data.insuranceClassId
              );
              if (insuranceClass) {
              ...
              if (itemDoc) {
                ...
                if (unitDoc) {
                  ...
                  if (storeDoc) {
                    ...
                    if (
                      itemDoc.workByBatch ||
                      itemDoc.workBySerial ||
                      itemDoc.workByQrCode
                    ) {
                ...
                item.totalAfterDiscounts =
                  item.totalPrice - item.totalDiscounts;
                if (!item.noVat) {
                  item.vat =
                    storesSetting.storesSetting &&
                    storesSetting.storesSetting.vat
                      ? storesSetting.storesSetting.vat
                      : 0;
                ...
                if (item.itemsMedicalTypes && obj.insuranceContract) {
                  if (
                    item.itemsMedicalTypes.id == 1 &&
                    obj.insuranceContract.appliesGeneric == "yes"
                  ) {
                    item.deduct =
                      (total * obj.insuranceContract.deductGeneric) / 100;
                    item.maxDeduct = obj.insuranceContract.maxAmountGeneric;
                    if (
                      item.deduct + salesInvoicesData.totalGeneric >
                      obj.insuranceContract.maxAmountGeneric
                    ) {
                      let remain =
                        item.deduct +
                        salesInvoicesData.totalGeneric -
                        obj.insuranceContract.maxAmountGeneric;
                    ...
                    item.itemsMedicalTypes.id == 2 &&
                    obj.insuranceContract.appliesBrand == "yes"
                  ) {
                    item.deduct =
                      (total * obj.insuranceContract.deductBrand) / 100;
                    item.maxDeduct = obj.insuranceContract.maxAmountBrand;
                    if (
                      item.deduct + salesInvoicesData.totalBrand >
                      obj.insuranceContract.maxAmountBrand
                    ) {
                      let remain =
                        item.deduct +
                        salesInvoicesData.totalBrand -
                        obj.insuranceContract.maxAmountBrand;
                      item.companyCash = item.total - (item.deduct + remain);
                      item.total = item.deduct - remain;
                    } else {
                    ...
                    item.itemsMedicalTypes.id == 3 &&
                    obj.insuranceContract.appliesMedicalDevice == "yes"
                  ) {
                    item.deduct =
                      (total * obj.insuranceContract.deductMedicalDevice) / 100;
                    item.maxDeduct =
                      obj.insuranceContract.maxAmountMedicalDevice;
                    if (
                      item.deduct + salesInvoicesData.totalMedicalDevice >
                      obj.insuranceContract.maxAmountMedicalDevice
                    ) {
                      let remain =
                        item.deduct +
                        salesInvoicesData.totalMedicalDevice -
                        obj.insuranceContract.maxAmountMedicalDevice;
                      item.companyCash = item.total - (item.deduct + remain);
                      item.total = item.deduct - remain;
                    } else {
                ...
                item.totalAfterDiscounts = site.toNumber(
                  item.totalAfterDiscounts
                );
                item.totalVat = site.toNumber(item.totalVat);
        ...
        (err, docs) => {
          for (let i = 0; i < docs.length; i++) {
            docs[i].unitsList.forEach((_unit) => {
              _unit.purchasePriceList = [
                { price: _unit.purchasePrice, date: new Date() },
              ];
              _unit.salesPriceList = [
                { price: _unit.salesPrice, date: new Date() },
              ];
              _unit.storesList = [];
