app.controller('reportHasTransactionPurchaseRequests', function ($scope, $http, $timeout) {
  $scope.baseURL = '';
  $scope.appName = 'reportHasTransactionPurchaseRequests';
  $scope.list = [];
  $scope.customersList = [];
  $scope.item = {};
  $scope._search = {};
  $scope.showView = function (_item) {
    ...
    $scope.item = {};
    $scope.view(_item);
    site.showModal('#purchaseRequestsManageModal');
  };
  ...
  $scope.getAll = function (where) {
    ...
    $http({
      method: 'POST',
      url: `/api/purchaseRequests/report`,
      data: {
        where: where,
        select: {
          id: 1,
