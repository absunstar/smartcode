app.controller('reportEmployeesPenalties', function ($scope, $http, $timeout) {
  $scope.baseURL = '';
  $scope.appName = 'reportEmployeesPenalties';
  $scope.list = [];
  $scope.item = {};
  $scope._search = {};
  $scope.showView = function (_item) {
    ...
    $scope.item = {};
    $scope.view(_item);
    site.showModal('#employeesPenaltiesManageModal');
  };
  ...
  $scope.view = function (_item) {
    ...
    $http({
      method: 'POST',
      url: `/api/employeesPenalties/view`,
      data: {
        id: _item.id,
      },
    }).then(
  ...
  $scope.getAll = function (where) {
    ...
    $http({
      method: 'POST',
      url: `/api/employeesPenalties/all`,
      data: {
        where: where,
        select: {
          id: 1,
