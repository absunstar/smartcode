  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Services Orders"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
    ...
    app.update(_data, (err, result) => {
      if (!err) {
        ...
        let objVoucher = {
          ...
          doctor: result.doc.doctor,
          patient: result.doc.patient,
          itemsList: result.doc.servicesList,
