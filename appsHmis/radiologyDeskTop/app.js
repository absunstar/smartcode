  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Radiology DeskTop"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
    ...
    if (app.allowRouteAll) {
      site.post({ name: `/api/${app.name}/all`, public: true }, (req, res) => {
        ...
        let select = req.body.select || { id: 1, patient: 1, doctor: 1, code: 1, detectionNum: 1, service: 1, treatment: 1, mainInsuranceCompany: 1, date: 1, status: 1 };
        let list = [];
        if (app.allowMemory) {
          app.memoryList
