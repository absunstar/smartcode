  ...
  if (app.allowRoute) {
    if (app.allowRouteGet) {
      site.get(
        ...
        (req, res) => {
          res.render(app.name + '/index.html', { title: app.name, appName: req.word("Offers Orders"), setting: site.getCompanySetting(req) }, { parser: 'html', compres: true });
        }
      );
    }
          ...
          if (!err && doc) {
            ...
            if (doc.amountPaid) {
              let obj = {
                date: new Date(),
                patient: doc.patient,
                voucherType: site.vouchersTypes[7],
                invoiceId: doc.id,
